...
var Edumin = function(){
	...
	var handleAllChecked = function() {
		...
		$(".checkAllInput").on('click',function() {
			...
			checkContentTitle($(this).closest('.filter'));
		});
		...
		$(".ItemsCheckboxSec input[type='checkbox']").on('change', function() {
			checkContentTitle($(this).closest('.filter'));
		});

		$(".filter .content-title input[type='checkbox']").on('click', function() {
			var isChecked = $(this).prop('checked');
			$(this).closest('.filter').find('.ItemsCheckboxSec input[type="checkbox"]').prop('checked', isChecked);
		});
	}

	var checkContentTitle = function(filterElement) {
		var totalCheckboxes = filterElement.find('.ItemsCheckboxSec input[type="checkbox"]').length;
        var checkedCheckboxes = filterElement.find('.ItemsCheckboxSec input[type="checkbox"]:checked').length;

        var contentTitleCheckbox = filterElement.find('.content-title input[type="checkbox"]');

        if (totalCheckboxes === checkedCheckboxes) {
            contentTitleCheckbox.prop('checked', true);
        } else {
            contentTitleCheckbox.prop('checked', false);
        }
	}
    ...
    /* Update the current date and time every 1 second */
    setInterval(() => {
        const date = new Date();
